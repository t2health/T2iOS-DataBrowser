...
-(Response *)handleResponse:(int)responseCode body:(NSString *)responseBody
{
    if (responseCode == 200) {
        ...
        return [[GetTokenResponse alloc] initWithAccessKey:accessKey andSecretKey:secretKey andSecurityToken:securityToken andExpirationDate:expirationDate];
    }
    else {
        return [[GetTokenResponse alloc] initWithCode:responseCode andMessage:responseBody];
    }
}
